...
module.exports = {
	encryptData: function(key, data){
		...
		var cipher = crypto.createCipher(encMode, key);
			cipher.update(data, 'hex', 'hex');
		return cipher.final('hex');
	},
	decryptData: function(key, data){

		var decipher = crypto.createDecipher(encMode, key);
		console.log("cipher created, prepping update for", data);

		// var updatedData = decipher.update(new Buffer(data, 'hex'));
		var updatedData = decipher.update(data, 'hex', 'ascii');
		// console.log("updated data=", updatedData.toString('utf8'));
		console.log("priting updated", updatedData);
